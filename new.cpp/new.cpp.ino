...
const int BUZZER_PIN = 5;      // Buzzer connected to digital pin D5
int ALCOHOL_THRESHOLD = 235;  // 160  230 // Adjust this threshold based on calibration
  ...
  if (irState == LOW) {  // Assuming LOW indicates an obstacle (depends on sensor type)
    ...
    if (tiltAngle < 143 || tiltAngle > 170) {
        ...
        //messafe to relatives emergency services
          Serial.println("  gyro tilt sensor!");
      buzz();
...
//Serial.println(temperature); 
  ...
  delay(1000); // Delay for half a second before taking the next reading
}
